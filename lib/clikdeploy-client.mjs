...
export function buildLinuxInstallerCommand(apiUrl, token, serverId, agentId) {
  ...
  return `curl -fsSL \"${base}/install.sh\" | bash -s -- --token \"${token}\" --server-id \"${serverId}\"${agentArg} --server \"${base}\"`;
}

export function buildWindowsInstallerCommand(apiUrl, token, serverId, agentId) {
  const base = normalizeApiUrl(apiUrl);
  const agentArg = agentId ? ` -AgentId '${agentId}'` : '';
  return `(irm \"${base}/install.ps1\") | iex; Install-ClikDeployAgent -Token '${token}' -ServerId '${serverId}'${agentArg} -ServerUrl '${base}'`;
}
...
export function buildDomainUrl(app) {
  const domain = app && typeof app.domain === 'string' ? app.domain.trim() : '';
