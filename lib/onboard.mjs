...
async function waitForInstallConfirmation(apiUrl, token, wait) {
  ...
  const url = `${normalizeApiUrl(apiUrl)}/api/agents/install-report?token=${encodeURIComponent(token)}${query}`;
...
export async function performAutoOnboard({
  ...
  try {
    ...
    const linuxCommand = buildLinuxInstallerCommand(apiUrl, token, serverId, agentId);
    const windowsCommand = buildWindowsInstallerCommand(apiUrl, token, serverId, agentId);
    const selectedCommand = platform === 'win32' ? windowsCommand : linuxCommand;
